    ...
    <fieldset id="titulo_frame">
      ...
      <table width="620" border="0" cellspacing="0" cellpadding="2" class="texto1" id="aba_produtos">
        ...
        <tr>
          ...
          <td><input type="number" step="1" minvalue="1" name="qtd_cc" tabindex="11" id="qtd_cc" onkeyup="if (window.event){tecla = window.event.keyCode;}else{tecla = event.which;}if (tecla == 13) {document.ped.ipi_cc.focus(); }" dir="rtl" value="<? echo $QtdItens;?>"  type="text" size="6" maxlength="10" onfocus="this.select()" onblur="if (document.ped.codigo_cc.value){return calculavalor(this.value)}"></td>
          <td valign="top" align="right">Ipi</td>
          <td valign="top">
            <input type="text" name="ipi_cc" tabindex="12" dir="rtl" value="<? echo $i[ipi];?>" id="ipi_cc" size="6" onkeyup="if (window.event){tecla = window.event.keyCode;}else{tecla = event.which;}if(tecla==13){document.ped.valor_unitario_cc.focus();}">
          </td>
          <td align="right">Valor :</td>
          <td><input name="valor_unitario_cc" dir="rtl" id="valor_unitario_cc" value="<? echo $ValorUnitario1;?>" type="text" size="8" maxlength="20" onfocus="this.select()" onblur="calculavalor();" onkeyup="if (window.event){tecla = window.event.keyCode;}else{tecla = event.which;}if(tecla==13){document.getElementById('Ok').style.display='block';document.ped.Ok.focus();}"></td>
          <td align="right">Total :</td>
          <td>
          <input name="valor_total_cc" id="valor_total_cc" value="<? echo $ValorTotal;?>" type="button" style="width: 90px; border: 1px solid #aba8a8; background-color:#FFFFFF; text-align: right;" size="8" maxlength="20" onfocus="this.select()" onblur="calculavalor();" onkeyup="if (window.event){tecla = window.event.keyCode;}else{tecla = event.which;}if(tecla==13){document.getElementById('Ok').style.display='block';document.ped.Ok.focus();}">
          </td>
        </tr>
        <tr>
          <td width="70" width="top">Descrição:</td>
          <td colspan="8" width="top">
            <input type="text" size="65" name="descricao_cc" value="<? echo $Nome;?>" id="descricao_cc" onfocus="this.select()" onkeyup="Acha1('listar.php','tipo=descricao&valor='+this.value+'','listar_descricao');">
            <?
            if ($_SESSION['config']['vendas']['UltimosItensPedido']){
              ?>
              <a href="#" OnClick="document.getElementById('UltimosItens').style.display='block';Acha1('vendas/ultimos_itens_pedido.php','IdCliente='+document.getElementById('cliente_id').value+'<? echo "$ListaNumero";?>&CodProd='+document.getElementById('codigo_cc').value+'','UltimosItens');"><input type="button" name="ult_itens" id="ult_itens" value="Ult. Ítens"></a>
              <?
            }
            ?>
            <BR>
        ...
        if ($ProdutoInexistente){
          ?>
          <tr>
            <td colspan="10"><? echo "$ProdutoInexistente";?></td>
          </tr>
          <?
        }
        ?>
